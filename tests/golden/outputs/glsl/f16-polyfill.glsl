...
Struct_4 wgsl_test_direct(float scalar_f16, float scalar_f32, vec2 vec2_f16, vec2 vec2_f32, vec3 vec3_f16, vec3 vec3_f32, vec4 vec4_f16, vec4 vec4_f32) {
    Struct_4 output;
    output.scalar_f16 = (scalar_f16 + 0.0f);
    output.scalar_f32 = (scalar_f32 + 1.0f);
    output.vec2_f16 = (vec2_f16 + vec2(0.0f));
    output.vec2_f32 = (vec2_f32 + vec2(1.0f));
    output.vec3_f16 = (vec3_f16 + vec3(0.0f));
    output.vec3_f32 = (vec3_f32 + vec3(1.0f));
    output.vec4_f16 = (vec4_f16 + vec4(0.0f));
    output.vec4_f32 = (vec4_f32 + vec4(1.0f));
    return output;
}

Struct_4 wgsl_test_struct(Struct_4 input) {
    Struct_4 output;
    output.scalar_f16 = (input.scalar_f16 + 0.0f);
    output.scalar_f32 = (input.scalar_f32 + 1.0f);
    output.vec2_f16 = (input.vec2_f16 + vec2(0.0f));
    output.vec2_f32 = (input.vec2_f32 + vec2(1.0f));
    output.vec3_f16 = (input.vec3_f16 + vec3(0.0f));
    output.vec3_f32 = (input.vec3_f32 + vec3(1.0f));
    output.vec4_f16 = (input.vec4_f16 + vec4(0.0f));
    output.vec4_f32 = (input.vec4_f32 + vec4(1.0f));
    return output;
}

Struct_4 wgsl_test_copy_input(Struct_4 input_original) {
    Struct_4 input = input_original;
    Struct_4 output;
    output.scalar_f16 = (input.scalar_f16 + 0.0f);
    output.scalar_f32 = (input.scalar_f32 + 1.0f);
    output.vec2_f16 = (input.vec2_f16 + vec2(0.0f));
...
float wgsl_test_return_partial(Struct_4 input_original) {
    Struct_4 input = input_original;
    input.scalar_f16 = 0.0f;
    return input.scalar_f16;
}

Struct_4 wgsl_test_component_access(Struct_4 input) {
    Struct_4 output;
